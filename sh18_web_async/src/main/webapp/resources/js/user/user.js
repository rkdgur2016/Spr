                ...
                try {
                    ...
                    if(userList.length > 0){
                        userList.forEach(function(value,i){
                            ...
                            html += '<tr>';
                            html += '<td class="text-center">' + value.no + '</td>'; // user.no 값 직접 입력
                            html += '<td class="text-center">' + value.userId + '</td>';
                            html += '<td class="text-center">' + value.name + '</td>';
                            html += '<td class="text-center">' + value.password + '</td>';
					...
					//기존 paging은 지운다.
					//paging을 새로운 데이터로 그려준다.
					
					const pagerDiv = document.querySelector('#page-selection');
					pagerDiv.innerHTML = "";
					pagerDiv.innerHTML = page(totalCnt,pageNo,userForm,pageSize.value,10,url,"doRetrieveAjax");
					
					
					
                } catch (e) {
                    console.error("data가 null혹은 undefined입니다", e);
